...
class PHPZip
{
	...
	function Zip($dir, $file)
	{
		...
		{	
			...
			if (count($filelist)>0)
			{
				...
				$this->mk_file($this->zipFileName); //create holder file if none
    ...
    function file()
    {
		...
		/**
		 * Takes current zip and puts it in a passworded zip
		 *
		 * @return passworded zip file
		 *
		 * @access public
		 *
		 * @src: http://stackoverflow.com/questions/646195/create-an-encrypted-zip-archive-with-php
		 */
		
		function addPassword( $password, $filename) {
			$this->zipPasswordFileName = $filename;
			
			//exec("zip -P $password ".$this->zipPasswordFileName." tender/files/4344/* 2>&1", $cmd);
			exec("zip -P $password ".$this->zipPasswordFileName." ".$this->zipFileName."", $cmd);
