    ...
    protected function generateMap($class_name, $base_path = '')
    {
        ...
        foreach ($methods as $method) {
            ...
            if (preg_match_all(
                ...
                if (preg_match_all('/^(GET|POST|PUT|DELETE|HEAD|OPTIONS)[_]?/i',
                        $method_url, $matches)
                ) {
                    $http_method = strtoupper($matches[1][0]);
                    $method_url = substr($method_url, strlen($matches[0][0]));
                } else {
                    $http_method = 'GET';
                }
                //echo $http_method." ".$class_name."->".$method_url."\n";
                $url = $base_path
